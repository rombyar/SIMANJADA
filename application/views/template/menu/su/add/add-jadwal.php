		...
		<div class="modal-content">
			...
			<?php $attributes = array('class' => 'form-horizontal'); echo form_open_multipart($this->uri->uri_string(),$attributes); ?>
				<div class="modal-body">
					...
					<?php if($ddkm_id) { foreach($ddkm_id as $view1) { ?>
					<input value="<?php echo $view1->id_dkm;?>" name="id_dkm">
					<?php } } ?>
					<div class="form-group">
						<label class="col-md-2 control-label" >Masjid</label>
						<div class="col-sm-10">
							<select class="form-control" required name="id_masjid" >
								<option value="0" >-- Masjid --</option>
								<?php if($dmasjid) { foreach($dmasjid as $view1) { ?>
								<option value="<?php echo $view1->id_masjid;?>" ><?php echo $view1->nama_masjid;?></option>
								<?php } } ?>
							</select>
						</div>
					</div>
					...
					<?php if($level == 2){ ?>
						<?php if($ddkm_id){ foreach($ddkm_id as $dmi){ ?>
							<input type="hidden" name="id_dkm" value="<?php echo $dmi->id_dkm;?>">
						<?php } } ?>
						<?php if($dmasjidid){ foreach($dmasjidid as $dmi){ ?>
							<input type="hidden" name="id_masjid" value="<?php echo $dmi->id_masjid;?>">
						<?php } } ?>
					<?php } ?>
