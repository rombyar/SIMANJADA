...
class user extends CI_Controller
{
	...
	function index()
	{
		...
		if ($message = $this->session->flashdata('message')) {
			...
			$this->template->load('template/content', 'template/user/general_message',  $data);
		} 
		else 
		{
	...
	function login()
	{
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			if($this->session->flashdata('redirectToCurrent') == TRUE){ //testing
			...
			if ($this->tank_auth->is_max_login_attempts_exceeded($login)) {
				if ($data['use_recaptcha'])
					$this->form_validation->set_rules('recaptcha_response_field', 'Konfirmasi Kode', 'trim|xss_clean|required|callback__check_recaptcha');
				else
					$this->form_validation->set_rules('captcha', 'Konfirmasi Kode', 'trim|xss_clean|required|callback__check_captcha');
			}
			$data['errors'] = array();
			...
			if ($this->form_validation->run()) {								// validation ok
				if ($this->tank_auth->login(
					...
					if (isset($errors['banned'])) {								// banned user
						...
						$this->_show_message($this->lang->line('auth_message_banned').' '.$errors['not_activated']);
						//redirect('/user/send_again/');
						//echo "Not activated 2";
			...
			$this->template->load('template/content', 'template/user/login_form', $data);
		}
	}
		...
		{						// logged in, not activated
			...
			{								// validation ok
				if (!is_null($data = $this->tank_auth->create_user( //Model create_user
						...
						//} else {
							//if ($this->config->item('email_account_details', 'tank_auth')) {	// send "" email
							//	$this->_send_email('', $data['email'], $data);
							//}
							unset($data['password']); // Clear password (just for any case)
							$this->_show_message($this->lang->line('auth_message_registration_completed_2').' '.anchor('/user/login/', 'Login'));
						//}
			...
			if ($captcha_registration) {
				if ($use_recaptcha) { $data['recaptcha_html'] = $this->_create_recaptcha();} 
				else { $data['captcha_html'] = $this->_create_captcha(); }
			}
			$data['use_username'] = $use_username;
			$data['captcha_registration'] = $captcha_registration;
			$data['use_recaptcha'] = $use_recaptcha;
			
			$this->template->load('template/content', 'template/user/register_form', $data);
		}
	}
	...
	function send_again()
	{
		...
		if (!$this->tank_auth->is_logged_in(FALSE)) {							// not logged in or activated
			...
			$this->template->load('template/content', 'template/user/send_again_form', $data);
		}
	}
	...
	function forgot_password()
	{
		...
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->template->load('template/content', 'template/user/forgot_password_form', $data);
		}
	}
	...
	function reset_password()
	{
		...
		$this->template->load('template/content', 'template/user/reset_password_form', $data);
	}
		...
		if (!$this->tank_auth->is_logged_in()) { redirect('/user/login/'); } 
			...
			if ($this->form_validation->run()) {
				...
				if ($this->tank_auth->change_password(
					...
					$errors = $this->tank_auth->get_error_message();
					foreach ($errors as $k => $v)	echo $data['errors'][$k] = $this->lang->line($v);
				}
				redirect('user/settings', $data);
			}else{
			$this->template->load('template/content', 'template/menu/su/su-settings', $data);
			}
		}
	}*/
	...
	function settings(){
		...
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->template->load('template/content', 'template/menu/su/su-settings', $data);
			//$this->template->load('template/content', 'template/user-member/member-myaccount', $data);
			//redirect('menu/myaccount', $data);
		}
	}
	...
	function change_email()
	{
		...
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->template->load('template/content', 'template/user/change_email_form', $data);
		}
	}
	...
	function unregister()
	{
		...
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->template->load('template/content', 'template/user/unregister_form', $data);
		}
	}
